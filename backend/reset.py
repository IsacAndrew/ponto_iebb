...
TABLES=('audit','days','items','schedules','settings','people','registros_ponto','usuarios')
...
def clear_system_data(db, actor):
    ...
    saved={column.name:getattr(actor,column.name) for column in Person.__table__.columns}
    ...
    if postgres:
        ...
        metadata=MetaData()
        for name in names: Table(name,metadata,autoload_with=connection)
        for table in reversed(metadata.sorted_tables):
            if table.name in names: connection.execute(table.delete())
