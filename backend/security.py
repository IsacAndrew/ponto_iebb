...
def promote(person):
    state=dict(person.session)
    if state.get('pending') and time.time() >= state['deadline']:
        state={'token':state['pending'],'expires':time.time()+43200}
...
def current(db, request: Request, allow_temporary=False):
    ...
    if not hmac.compare_digest(state.get('token',''),digest(token)) or state.get('expires',0)<time.time(): fail('Sua sessão foi encerrada. Entre novamente.',401)
...
def public(person):
    return dict(id=person.id,name=person.name,role=person.role,active=person.active,hired=person.hired,terminated=person.terminated,details=person.details,temporary=person.temporary)
