...
from .rules import fail, roles_for, effective_role
...
def current(db, request: Request, allow_temporary=False, allow_unselected=False):
    ...
    if len(roles_for(person))>1 and state.get('active_role') not in roles_for(person) and not allow_unselected: fail('Escolha o perfil de acesso.',409)
    return person
def require(person, roles):
    if effective_role(person) not in roles: fail('Você não tem acesso a esta ação.',403)
def confirm(person, password):
    if not verify(password,person.password): fail('Senha incorreta.',403)
def public(person, active=False):
    assigned=roles_for(person); role=effective_role(person) if active else person.role
    return dict(id=person.id,name=person.name,login=person.login,role=role,roles=assigned,access_required=active and len(assigned)>1 and (person.session or {}).get('active_role') not in assigned,active=person.active,hired=person.hired,terminated=person.terminated,details=person.details,temporary=person.temporary)
