...
if os.getenv("RENDER") and url.startswith("sqlite"):
    raise RuntimeError("Configure DATABASE_URL PostgreSQL no Render.")
...
engine = create_engine(
    url, pool_pre_ping=True, hide_parameters=True, connect_args=connection_args(url)
)
...
def init():
    ...
    with engine.begin() as connection:
        if engine.dialect.name == "postgresql":
            connection.execute(text("SELECT pg_advisory_xact_lock(730030)"))
            ...
            for table in Base.metadata.sorted_tables:
                ...
                for role in ("anon", "authenticated"):
                    if connection.scalar(
                        ...
                        connection.execute(
                            text(f'REVOKE ALL ON "{table.name}" FROM {role}')
                        )
    ...
    with Session(engine, expire_on_commit=False) as db:
        try:
            ...
            if engine.dialect.name == "sqlite":
                ...
                db.execute(
                    select(Setting).where(Setting.key == "mutex").with_for_update()
                ).scalar_one()
            yield db
            db.commit()
        except Exception:
