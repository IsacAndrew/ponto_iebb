...
from .db import Day, Schedule, Setting, today
...
def validate_days(days):
    if not isinstance(days, dict) or set(days) - set(map(str, range(7))):
        fail("Jornada inválida.")
    for periods in days.values():
        ...
        for pair in periods:
            if not isinstance(pair, (list, tuple)) or len(pair) != 2:
                fail("Informe entrada e saída.")
            try:
                start, end = map(minutes, pair)
            except (ValueError, TypeError):
                fail("Horário inválido.")
            if start >= end or start <= previous:
                fail("Os períodos devem estar em ordem, sem sobreposição.")
            previous = end
    return days


def schedule_for(db, person, day):
    cache = db.info.setdefault("schedules", {})
    if person.id not in cache:
        cache[person.id] = list(
            db.scalars(
                select(Schedule)
    ...
    rows = [s for s in cache[person.id] if s.effective <= day]
    chosen = next((s for s in rows if s.specific and s.effective == day), None) or next(
        (s for s in rows if not s.specific), None
    )
    return chosen.days.get(str(date.fromisoformat(day).weekday()), []) if chosen else []


def holiday(db, day):
    cache = db.info.setdefault("holidays", {})
    if day not in cache:
...
def get_day(db, person, day, create=False):
    ...
    if not row and create:
        row = Day(
            ...
            person_id=person.id,
            date=day,
            periods=schedule_for(db, person, day),
            punches=[],
            absent=False,
            overtime="",
        )
...
def summarize(db, person, day):
    ...
    periods = (
        row.periods
        if row and (row.punches or day < today())
        else schedule_for(db, person, day)
    )
    ...
    worked = sum(
        max(0, minutes(punches[i + 1]["time"]) - minutes(punches[i]["time"]))
    ...
    status = free or (
        "Falta registrada"
        if absent and not punches
        else "Falta com comparecimento"
        if absent
        else "Sem expediente"
        if not expected and not punches
        else "Completo"
        if complete and punches
        else "Ponto incompleto"
        if punches
        else "Sem marcações"
    )
    if absent and not punches and not free:
        negative = planned
    balance = extra - negative if complete or absent or free else None
    assigned = roles_for(person)
    role = (
        punches[0].get("access_role")
        if punches and punches[0].get("access_role") in assigned
        else ("Professor" if "Professor" in assigned else person.role)
    )
    ...
    return dict(
        ...
        status=status,
        holiday=free,
        overtime=row.overtime if row else "",
...
def distance(lat, lon, target_lat, target_lon):
    ...
    v = (
        sin((b - a) / 2) ** 2
        + cos(a) * cos(b) * sin(radians(target_lon - lon) / 2) ** 2
    )
    return 6371000 * 2 * atan2(sqrt(v), sqrt(max(0, 1 - v)))


def suspect_missing(expected, index, minute):
    # A sequência precisa ter avançado, não apenas haver atraso.
    if index >= len(expected):
        return False
    target = minutes(expected[index])
    if minute <= target + 60:
        return False
    if index + 1 < len(expected):
        return minute >= minutes(expected[index + 1]) - 5
    return minute >= target + 180
