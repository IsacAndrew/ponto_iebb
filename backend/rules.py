...
def schedule_for(db, person, day):
    ...
    if person.id not in cache:
        cache[person.id] = list(db.scalars(select(Schedule).where(Schedule.person_id == person.id).order_by(Schedule.effective.desc(), Schedule.id.desc())))
...
def summarize(db, person, day):
    ...
    periods = row.periods if row and (row.punches or day < today()) else schedule_for(db, person, day)
...
def suspect_missing(expected, index, minute):
    ...
    if index >= len(expected): return False
    target = minutes(expected[index])
    if minute <= target + 60: return False
