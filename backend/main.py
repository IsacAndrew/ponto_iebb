import os, time, secrets, json, math, base64
...
from .db import ROOT, init, transaction, Person, Schedule, Day, Item, Setting, Audit, audit, now, today
from .rules import ROLES, ADMIN, CLASSES, fail, minutes, valid_date, validate_days, schedule_for, holiday, unlocked, get_day, summarize, distance
from .security import password_hash, verify, digest, cookie, promote, current, require, confirm, public
...
def login(response: Response, data: dict=Body(...)):
    ...
    with transaction() as db:
        ...
        if not person or not verify(str(data.get('password','')),person.password):
            ...
            rate.data={}
            token=f'{person.id}:'+secrets.token_urlsafe(32)
            state=promote(person)
            if state.get('token') and state.get('expires',0)>time.time():
                if state.get('pending'): return JSONResponse({'detail':'Há outro acesso aguardando. Tente novamente em alguns segundos.'},status_code=409)
                person.session={**state,'pending':digest(token),'deadline':time.time()+5}
                result={'pending':True,'ticket':token}
            else:
...
def claim(response: Response,data: dict=Body(...)):
    token=data.get('ticket','')
    with transaction() as db:
        try: person=db.get(Person,int(token.split(':')[0]))
        except (ValueError,IndexError): fail('Acesso expirado.',401)
        if not person or not person.active: fail('Acesso expirado.',401)
        state=promote(person)
        if state.get('pending')==digest(token): return {'pending':True}
        if state.get('token')!=digest(token): fail('O novo acesso foi cancelado no outro dispositivo.',409)
        cookie(response,token)
        return {'user':public(person)}
@app.get('/api/me')
def me(request: Request):
    with transaction() as db:
        person=current(db,request,True)
        return {'user':public(person),'takeover':person.session.get('deadline'),'now':now().isoformat(),'test_bypass':bypass(person)}
...
def punch_today(request: Request):
    with transaction() as db:
        ...
        return {**summarize(db,p,today()),'now':now().isoformat(),'test_bypass':bypass(p)}
@app.post('/api/punch')
def punch(request: Request,data: dict=Body(...)):
    with transaction() as db:
        ...
        unusual=not extra and abs(minute-minutes(expected[index]))>60
        ...
        geo=db.get(Setting,'geo').data
        location={}
        if not test:
            if not geo.get('verified'): fail('Localização da escola aguarda confirmação do Suporte.',422)
...
def editable(actor,target,role=None):
    if (target and target.role=='Suporte' or role=='Suporte') and actor.role!='Suporte': fail('Somente Suporte gerencia esse perfil.',403)
    if (target and target.role=='Diretoria' or role=='Diretoria') and actor.role=='Administração': fail('Somente Diretoria ou Suporte gerenciam esse perfil.',403)
@app.post('/api/people')
...
def tickets(request:Request):
    with transaction() as db:
        actor=current(db,request)
        return [item_json(db,r) for r in db.scalars(select(Item).where(Item.kind=='ticket').order_by(Item.id.desc())) if actor.role=='Suporte' or r.person_id==actor.id]
@app.post('/api/tickets')
def new_ticket(request:Request,data:dict=Body(...)):
    with transaction() as db:
        actor=current(db,request); message=str(data.get('message','')).strip()
        ...
        row.data={'messages':row.data['messages']+[{'name':actor.name,'support':actor.role=='Suporte','text':message,'at':now().isoformat()}]}
        return {'ok':True}
...
def message(rid:int,request:Request,data:dict=Body(...)):
    with transaction() as db:
        ...
        row.data={'messages':row.data['messages']+[{'name':actor.name,'support':actor.role=='Suporte','text':value,'at':now().isoformat()}]}
        return {'ok':True}
@app.delete('/api/tickets/{rid}')
def close_ticket(rid:int,request:Request):
    with transaction() as db:
        require(current(db,request),['Suporte']); row=db.get(Item,rid)
...
def settings(request:Request):
    with transaction() as db:
        require(current(db,request),['Suporte'])
        return db.get(Setting,'geo').data
@app.put('/api/support/settings')
def save_settings(request:Request,data:dict=Body(...)):
    with transaction() as db:
        actor=current(db,request); require(actor,['Suporte']); confirm(actor,data.get('password',''))
        ...
        row=db.get(Setting,'geo'); before=row.data
        row.data={'lat':lat,'lon':lon,'accuracy':accuracy,'verified':data.get('verified') is True}
