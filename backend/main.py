...
def reset(pid:int,request: Request):
    with transaction() as db:
        actor=current(db,request); require(actor,FULL_ACCESS); p=db.get(Person,pid)
        if not p: fail('Pessoa não encontrada.',404)
...
def favicon():
    with reading() as db:
        ...
        if not row: return RawResponse(status_code=204)
        return RawResponse(base64.b64decode(row.data['content']),media_type=row.data['type'],headers={'Cache-Control':'public, max-age=300'})
@app.put('/api/favicon')
def save_favicon(request:Request,data:dict=Body(...)):
    with transaction() as db:
        actor=current(db,request); require(actor,FULL_ACCESS)
        content=str(data.get('content','')); kind=str(data.get('type',''))
        if kind not in ('image/png','image/jpeg'): fail('Use uma imagem PNG, JPG ou JPEG.')
        try: raw=base64.b64decode(content,validate=True)
        except Exception: fail('Imagem inválida.')
        if not raw or len(raw)>512000: fail('Use uma imagem de até 500 KB.')
        if kind=='image/png' and not raw.startswith(b'\x89PNG') or kind=='image/jpeg' and not raw.startswith(b'\xff\xd8'): fail('O conteúdo não corresponde ao formato da imagem.')
        row=db.get(Setting,'favicon')
        if row: row.data={'type':kind,'content':content}
        else: db.add(Setting(key='favicon',data={'type':kind,'content':content}))
        audit(db,actor,'Alterar favicon','sistema'); return {'ok':True}
@app.delete('/api/favicon')
def remove_favicon(request:Request):
    with transaction() as db:
        actor=current(db,request); require(actor,FULL_ACCESS); row=db.get(Setting,'favicon')
        if row: db.delete(row); audit(db,actor,'Remover favicon','sistema')
        return {'ok':True}
@app.get('/api/today-events')
def today_events(request:Request):
    with reading() as db:
        require(current(db,request),ADMIN); events=[]
        for row in db.scalars(select(Day).where(Day.date==today())):
            person=db.get(Person,row.person_id)
            events += [{'time':x['time'],'name':person.name,'person_id':person.id} for x in row.punches]
