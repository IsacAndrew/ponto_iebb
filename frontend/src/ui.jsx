...
export async function api(path,data,method){
 const r=await fetch('/api'+path,{method:method||(data===undefined?'GET':'POST'),headers:{'Content-Type':'application/json','X-Ponto':'1'},credentials:'same-origin',...(data!==undefined?{body:JSON.stringify(data)}:{})});
 if(!r.ok){const j=await r.json().catch(()=>({}));const e=new Error(typeof j.detail==='string'?j.detail:'Não foi possível concluir. Tente novamente.'); e.status=r.status;throw e;}
 return r.headers.get('content-type')?.includes('spreadsheet')?r.blob():r.json();
}
