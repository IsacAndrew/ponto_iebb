...
export function Modal({title,close,children,wide=false}){
 const ref=useRef(),successTimer=useRef(),[error,setError]=useState(''),[success,setSuccess]=useState('');useEffect(()=>{const old=document.activeElement;ref.current.showModal();const handleError=e=>setError(e.detail),handleSuccess=e=>{setSuccess(e.detail);clearTimeout(successTimer.current);successTimer.current=setTimeout(()=>setSuccess(''),4000)};window.addEventListener('ponto-error',handleError);window.addEventListener('ponto-success',handleSuccess);return()=>{window.removeEventListener('ponto-error',handleError);window.removeEventListener('ponto-success',handleSuccess);clearTimeout(successTimer.current);old?.focus();};},[]);
 return <dialog ref={ref} className={wide?'wide':''} onCancel={e=>{e.preventDefault();close();}}><header><h2>{title}</h2><button className="icon" aria-label="Fechar" onClick={close}><X/></button></header>{error&&<div className="inline-error" role="alert">{error}<button onClick={()=>setError('')}>×</button></div>}{success&&<div className="inline-success" role="status">{success}<button onClick={()=>setSuccess('')}>×</button></div>}{children}</dialog>;
}
