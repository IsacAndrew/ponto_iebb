...
function App(){
 ...
 if(!me)return <><Login onLogin={p=>{generation.current++;setMe(p);setPage(p.role==='Diretoria'?'Usuários':'Ponto');}} run={run} busy={busy}/>{error&&<div className="toast error" role="alert">{error}<button onClick={()=>setError('')}>×</button></div>}</>;
