...
os.environ["BOOTSTRAP_PASSWORD"] = "bootstrap-test-password"
test_database = "sqlite:///" + str(
    Path(os.getenv("TEMP", ".")) / ("ponto-test-" + uuid.uuid4().hex + ".db")
)
if os.getenv("PONTO_TEST_POSTGRES"):
    # Only a dedicated test database is accepted; never reuse DATABASE_URL.
    test_database = os.environ["PONTO_TEST_POSTGRES"]
    if not test_database.split("?")[0].endswith("/ponto_test"):
        raise RuntimeError("Use um banco descartável chamado ponto_test.")
...
ROUTES = (main, auth, attendance, people, requests, support, system, reports)
from backend.db import Audit, Day, Item, Person, Schedule, Setting, transaction
...
def cleanup_test_database():
    db.engine.dispose()
    if test_database.startswith("sqlite:///"):
        Path(test_database.removeprefix("sqlite:///")).unlink(missing_ok=True)
...
def clock(monkeypatch):
    ...
    for module in ROUTES:
        ...
        monkeypatch.setattr(
            module, "today", lambda: NOW.date().isoformat(), raising=False
        )
...
def client():
    with TestClient(app) as c:
        with transaction() as session:
            ...
            session.add(
                Person(
                    ...
                    name="Isac",
                    login="suporte",
                    password=password_hash("definitiva1"),
                    role="Suporte",
                    temporary=False,
                    active=True,
                    hired="2026-01-01",
        ...
        assert (
            c.post(
                "/api/login", json={"login": "suporte", "password": "definitiva1"}
            ).status_code
            == 200
        )
        yield c
    ...
    with transaction() as s:
        s.add(
            Schedule(
                ...
                effective=day,
                specific=False,
                days={str(i): periods for i in range(7)},
            )
        )
...
def at(monkeypatch, value):
    ...
    for module in ROUTES:
        monkeypatch.setattr(module, "now", lambda: t, raising=False)
        monkeypatch.setattr(
            module, "today", lambda: t.date().isoformat(), raising=False
        )
    monkeypatch.setattr(db, "now", lambda: t)
...
def test_login_first_password_and_masked_profile(client):
    ...
    with transaction() as s:
        p = s.get(Person, pid)
        p.temporary = True
        p.password = password_hash("102030")
    with TestClient(app) as c:
        c.headers["X-Ponto"] = "1"
        assert c.post(
        ...
        assert (
            c.post("/api/profile/login", json={"password": "errada"}).status_code == 403
        )
...
def test_tolerance_early_exit_and_idempotency(client, monkeypatch):
    ...
    at(monkeypatch, "2026-09-03T06:54:59")
    assert punch(client).status_code == 400
    at(monkeypatch, "2026-09-03T07:06:00")
    key = uuid.uuid4().hex
    ...
    at(monkeypatch, "2026-09-03T14:40:00")
    punch(client)
...
def test_all_periods_overtime_midnight(client, monkeypatch):
    ...
    for t in ["05:00", "10:00", "11:00", "15:06", "17:00", "20:05"]:
        at(monkeypatch, "2026-09-03T" + t + ":00")
        assert punch(client).status_code == 200
    r = client.get("/api/punch/today").json()
    assert r["extra"] == 1 and r["expected"] == 6
    at(monkeypatch, "2026-09-03T20:10:00")
    assert punch(client).json()["question"] == "overtime"
    assert punch(client, overtime=True).status_code == 200
    at(monkeypatch, "2026-09-03T20:40:00")
    assert punch(client, overtime=True).status_code == 200
    assert client.get("/api/punch/today").json()["extra"] == 31
    at(monkeypatch, "2026-09-04T05:00:00")
    assert punch(client).status_code == 200
    assert len(client.get("/api/punch/today").json()["punches"]) == 1
...
def test_forgot_and_absence_arrival(client, monkeypatch):
    ...
    assert (
        client.post(
            "/api/absence", json={"person_id": 1, "date": "2026-09-03", "absent": True}
        ).status_code
        == 200
    )
    at(monkeypatch, "2026-09-03T15:00:00")
    assert punch(client).json()["question"] == "forgot"
    r = punch(client, forgot="yes")
    assert r.status_code == 200 and "falta registrada" in r.json()["message"]
    occurrences = client.get("/api/support/occurrences").json()
    assert {x["data"]["title"] for x in occurrences} >= {
        "Batida possivelmente esquecida",
        "Falta registrada com comparecimento",
    }
    assert (
        client.post(
            "/api/absence", json={"person_id": 1, "date": "2026-09-03", "absent": False}
        ).status_code
        == 200
    )
...
def test_geofence_and_permissions(client):
    ...
    with TestClient(app) as c:
        c.headers["X-Ponto"] = "1"
        c.post("/api/login", json={"login": "professor", "password": "definitiva1"})
        assert c.post("/api/punch", json={"key": uuid.uuid4().hex}).status_code == 422
        assert (
            punch(c, lat=-23.67637077, lon=-46.76243126, accuracy=501).status_code
            == 422
        )
        assert punch(c, lat=-23.68, lon=-46.77, accuracy=10).status_code == 422
        assert (
            punch(c, lat=-23.67637077, lon=-46.76243126, accuracy=10).status_code == 200
        )
        assert (
            c.post(
                "/api/absence",
                json={"person_id": pid, "date": "2026-09-03", "absent": True},
            ).status_code
            == 403
        )
...
def test_schedule_history_immutable(client, monkeypatch):
    ...
    punch(client)
    r = client.post(
        "/api/people/1/schedules",
        json={
            "effective": "2026-09-03",
            "days": {"3": [["08:00", "16:00"]], "4": [["08:00", "16:00"]]},
        },
    )
    assert r.json()["effective"] == "2026-09-04"
    assert client.get("/api/punch/today").json()["periods"] == [["07:00", "15:00"]]
    at(monkeypatch, "2026-09-04T08:00:00")
    assert client.get("/api/punch/today").json()["periods"] == [["08:00", "16:00"]]
    assert (
        client.post(
            "/api/people/1/schedules", json={"effective": "2026-08-03", "days": {}}
        ).status_code
        == 400
    )
...
def test_download_excel_without_closing_and_holiday(client):
    pid = add_person()
    set_schedule(pid, [["07:00", "12:00"], ["13:00", "17:00"], ["18:00", "20:00"]])
    assert (
        client.post(
            "/api/calendar", json={"date": "2026-08-04", "name": "Feriado"}
        ).status_code
        == 200
    )
    assert (
        client.post(
            "/api/absence",
            json={"person_id": pid, "date": "2026-08-03", "absent": True},
        ).status_code
        == 200
    )
    assert (
        client.post(
            ...
            json={
                ...
                "date": "2026-08-05",
                "times": ["07:00"],
                "reason": "Batida esquecida",
            },
        ).status_code
        == 200
    )
    r = client.post("/api/month/2026-08/export", json={"password": "definitiva1"})
    assert r.status_code == 200
    first = r.content
    wb = load_workbook(BytesIO(first))
    assert len(wb.sheetnames) == 4
    sheet = wb["Pontos - Geral"]
    assert sheet.auto_filter.ref and sheet.freeze_panes == "E2"
    ...
    assert (
        client.post(
            "/api/calendar", json={"date": "2026-08-04", "name": ""}
        ).status_code
        == 200
    )
    assert (
    ...
    assert not any(
        c.value == "Feriado" for row in workbook["Pontos - Geral"] for c in row
    )
...
def test_tickets_delete_and_requests_audit(client):
    ...
    with TestClient(app) as c:
        ...
        c.post("/api/login", json={"login": "professor", "password": "definitiva1"})
        assert (
            c.post("/api/tickets", json={"message": "Preciso de ajuda"}).status_code
            == 200
        )
        assert client.get("/api/tickets/unread").json() == {
            "count": 1,
            "names": ["Pessoa professor"],
        }
        ticket = c.get("/api/tickets").json()[0]["id"]
        assert c.delete("/api/tickets/" + str(ticket)).status_code == 403
        assert (
            client.post(
                f"/api/tickets/{ticket}/message", json={"message": "Vou verificar"}
            ).status_code
            == 200
        )
        ...
        assert (
            client.post(
                f"/api/requests/{req}/decide",
                json={"approve": True, "reason": "Conferido"},
            ).status_code
            == 200
        )
        assert c.get("/api/me").json()["user"]["name"] == "Nome corrigido"
    assert any(
        r["action"] == "Aprovar alteração cadastral"
        for r in client.get("/api/audit?month=2026-09").json()
    )
...
def test_double_requests_and_inactivation(client):
    ...
    assert (
        client.post(
            f"/api/people/{pid}/deactivate", json={"date": "2026-09-03"}
        ).status_code
        == 200
    )
    assert (
        client.post(
            "/api/login", json={"login": "professor", "password": "definitiva1"}
        ).status_code
        == 401
    )
    with transaction() as s:
...
def test_pool_configuration():
    assert (
        db.connection_args("postgresql+psycopg://localhost/test")["prepare_threshold"]
        is None
    )
...
def test_geo_confirmation_persists(client):
    ...
    assert (
        punch(
            client, test=False, lat=settings["lat"], lon=settings["lon"], accuracy=10
        ).status_code
        == 409
    )
...
def test_top_profile_and_support_no_self_ticket(client):
    ...
    assert (
        client.put("/api/profile", json={"phone": "11 98564-1624"}).status_code == 200
    )
...
def test_requested_support_four_times(client, monkeypatch):
    ...
    result = client.post(
        "/api/people/1/schedules",
        json={"effective": "2026-09-03", "days": {"3": periods}},
    )
    ...
    result = client.post(
        "/api/people/1/schedules",
        json={"effective": "2026-09-03", "days": {"4": periods}},
    )
...
def test_schedule_query_reused_within_request(client):
    ...
    pid = add_person()
    set_schedule(pid, [["07:30", "11:00"], ["12:00", "17:30"]])
    queries = []

    def count(conn, cursor, statement, parameters, context, executemany):
        if "FROM schedules" in statement:
            queries.append(statement)

    event.listen(db.engine, "before_cursor_execute", count)
    try:
        result = client.get(
            f"/api/records?start=2026-08-01&end=2026-08-31&person_id={pid}"
        )
...
def test_global_qr_uses_authenticated_person(client, monkeypatch):
    pid = add_person(login="qruser")
    set_schedule(pid, [["07:00", "12:00"], ["13:00", "17:00"]])
    ...
    with TestClient(app) as anonymous:
        ...
        assert (
            anonymous.post(
                "/api/login", json={"login": "qruser", "password": "definitiva1"}
            ).status_code
            == 200
        )
        ...
        assert (
            ...
            and result.json()["confirmation"]
            == "Pessoa, entrada registrada com sucesso"
        )
...
def test_admission_and_password_permission(client):
    ...
    assert (
        next(x for x in client.get("/api/people").json() if x["id"] == pid)["hired"]
        == "1999-02-01"
    )
    add_person(role="Administração", login="middle")
    with TestClient(app) as c:
        c.headers["X-Ponto"] = "1"
        c.post("/api/login", json={"login": "middle", "password": "definitiva1"})
...
def test_temporary_password_allows_access_and_optional_change(client):
    ...
    with TestClient(app) as c:
        ...
        assert (
            c.post(
                "/api/login", json={"login": "temporary", "password": "102030"}
            ).status_code
            == 200
        )
        assert c.get("/api/me").status_code == 200
...
def test_recovery_email_single_use_expiry_and_cooldown(client, monkeypatch):
    ...
    with transaction() as db:
        p = db.get(Person, pid)
        p.details = {"email": "pessoa@example.test"}
    sent = []
    ...
    monkeypatch.setenv("SMTP_FROM", "sistema@example.test")
    ...
    assert (
        client.post("/api/recover", json={"login": "recoverme"}).json()["message"]
        == neutral
    )
    ...
    with transaction() as db:
        ...
        assert credential not in str(stored) and stored["hash"] == digest(credential)
    assert (
        client.post("/api/recover", json={"login": "recoverme"}).json()["message"]
        == neutral
        and len(sent) == 1
    ...
    with TestClient(app) as c:
        c.headers["X-Ponto"] = "1"
        response = c.post(
            "/api/login", json={"login": "recoverme", "password": credential}
        )
    ...
    with transaction() as db:
        rate = db.get(Setting, "recover-rate:" + digest("recoverme"))
        rate.data = {"until": 0}
    client.post("/api/recover", json={"login": "recoverme"})
    expired = sent[-1].get_content().splitlines()[0]
    with transaction() as db:
        row = db.get(Setting, f"recovery:{pid}")
        row.data = {**row.data, "expires": 0}
    with TestClient(app) as c:
        c.headers["X-Ponto"] = "1"
        assert (
            c.post(
                "/api/login", json={"login": "recoverme", "password": expired}
            ).status_code
            == 401
        )
...
def test_support_location_exception_is_per_account_and_audited(client):
    ...
    assert client.put("/api/support/my-location", json={"required": False}).json() == {
        "required": False
    }
    set_schedule(1, [["07:00", "15:00"]])
    assert client.post("/api/punch", json={"key": uuid.uuid4().hex}).status_code == 200
    other = add_person(role="Suporte", login="othersupport")
    set_schedule(other, [["07:00", "15:00"]])
    with TestClient(app) as c:
        c.headers["X-Ponto"] = "1"
        c.post("/api/login", json={"login": "othersupport", "password": "definitiva1"})
        assert c.get("/api/support/my-location").json() == {"required": True}
        assert c.post("/api/punch", json={"key": uuid.uuid4().hex}).status_code == 422
    assert any(
        a["action"] == "Alterar exigência de localização própria"
        for a in client.get("/api/audit?month=2026-09").json()
    )


def test_grade_times_are_manual_and_do_not_change_schedule(client, monkeypatch):
    pid = add_person(login="teacher")
    set_schedule(pid, [["07:10", "08:50"], ["09:10", "12:30"]])
    subjects = {
        "1º Ano A": ["Matemática"],
        "2º Ano A": ["Matemática"],
        "3º Ano A": ["Artes"],
    ...
    assert (
        client.put(f"/api/people/{pid}/lessons", json={"lessons": lessons}).status_code
        == 200
    )
    invalid = {**lessons[0], "end": "07:00"}
    assert (
        client.put(
            f"/api/people/{pid}/lessons", json={"lessons": [invalid]}
        ).status_code
        == 400
    )
    at(monkeypatch, "2026-09-07T07:10:00")
    with TestClient(app) as teacher:
        teacher.headers["X-Ponto"] = "1"
        teacher.post("/api/login", json={"login": "teacher", "password": "definitiva1"})
        point = teacher.get("/api/punch/today").json()
        assert (
            point["periods"] == [["07:10", "08:50"], ["09:10", "12:30"]]
            and point["expected"] == 4
        )
    storage = client.get("/api/system/storage")
    assert (
        storage.status_code == 200
        and storage.json()["used_bytes"] > 0
        and storage.json()["limit_bytes"] > 0
    )


def test_one_login_can_choose_professor_or_administrator(client, monkeypatch):
    pid = add_person(login="edson")
    set_schedule(pid, [["07:10", "08:50"]])
    with transaction() as db:
        person = db.get(Person, pid)
        person.details = {**person.details, "roles": ["Professor", "Administração"]}
    at(monkeypatch, "2026-09-03T07:10:00")
    with TestClient(app) as user:
        user.headers["X-Ponto"] = "1"
        login = user.post(
            "/api/login", json={"login": "edson", "password": "definitiva1"}
        ).json()
        ...
        assert (
            chosen["user"]["role"] == "Professor"
            and not chosen["user"]["access_required"]
        )
        ...
        assert (
            user.post("/api/access", json={"role": "Administração"}).json()["user"][
                "role"
            ]
            == "Administração"
        )
    ...
    workbook = load_workbook(
        BytesIO(
            client.post(
                "/api/month/2026-09/export", json={"password": "definitiva1"}
            ).content
        )
    )
...
def test_teacher_export_and_notebook_location(client):
    ...
    assert list(sheet.values) == [
        ...
        ("Pessoa exportteacher", "1º Ano A", "Português"),
        ("Pessoa exportteacher", "1º Ano A", "Matemática"),
        ("Pessoa exportteacher", "2º Ano A", "Arte"),
    ]
    ...
    with TestClient(app) as user:
        user.headers["X-Ponto"] = "1"
        user.post(
            "/api/login", json={"login": "exportteacher", "password": "definitiva1"}
        )
        assert user.get("/api/reports/teachers").status_code == 403
        assert punch(user, lat=-23.68, lon=-46.77, accuracy=400).status_code == 422
        assert (
            punch(user, lat=-23.67637077, lon=-46.76243126, accuracy=400).status_code
            == 200
        )
...
def test_grade_saves_new_subjects_atomically(client):
    ...
    assert (
        ...
        and "Segunda-feira, 1ª aula" in response.json()["detail"]
    )
    subjects = {"1º Ano A": ["Português"]}
    assert (
        client.put(
            f"/api/people/{pid}/lessons",
            json={"lessons": [lesson], "subjects": subjects},
        ).status_code
        == 200
    )
    with transaction() as db:
    ...
    response = client.put(
        f"/api/people/{pid}/lessons",
        json={
            "lessons": [lesson, invalid],
            "subjects": {"1º Ano A": ["Português", "Arte"]},
        },
    )
    assert (
        response.status_code == 400
        and "Terça-feira, 1ª aula" in response.json()["detail"]
    )
    with transaction() as db:
        saved = db.get(Person, pid).details
        assert saved["lessons"] == [lesson] and saved["subjects_by_class"] == subjects
...
def test_people_indicates_registered_schedule(client):
    missing = add_person(login="missing")
    empty = add_person(login="empty")
    registered = add_person(login="registered")
    set_schedule(empty, [])
    set_schedule(registered, [["07:10", "12:30"]], day="2026-10-01")
...
def test_database_reset_requires_support_and_confirmation(client):
    pid = add_person(login="resetteacher")
    set_schedule(pid, [["07:00", "12:00"]])
    assert (
        client.post(
            "/api/absence",
            json={"person_id": pid, "date": "2026-08-03", "absent": True},
    ...
    with TestClient(app) as teacher:
        teacher.headers["X-Ponto"] = "1"
        teacher.post(
            "/api/login", json={"login": "resetteacher", "password": "definitiva1"}
        )
        teacher.post("/api/tickets", json={"message": "Teste"})
        assert (
            teacher.post(
                "/api/system/reset",
                json={"password": "definitiva1", "confirmation": "APAGAR"},
            ).status_code
            == 403
        )
        assert (
            client.post(
                "/api/system/reset",
                json={"password": "errada", "confirmation": "APAGAR"},
            ).status_code
            == 403
        )
        assert (
            client.post(
                "/api/system/reset",
                json={"password": "definitiva1", "confirmation": ""},
            ).status_code
            == 400
        )
        with transaction() as db:
    ...
    with transaction() as db:
        ...
        for model in (Day, Schedule, Item):
...
def test_reset_clears_legacy_and_rolls_back_on_failure(client, monkeypatch):
    ...
    with transaction() as session:
        session.execute(
            text("CREATE TABLE usuarios (id INTEGER PRIMARY KEY, nome TEXT)")
        )
        session.execute(
            text(
                "CREATE TABLE registros_ponto (id INTEGER PRIMARY KEY, usuario_id INTEGER REFERENCES usuarios(id))"
            )
        )
    ...
    def reject(*args, **kwargs):
        raise RuntimeError("Falha simulada antes de confirmar a limpeza")

    monkeypatch.setattr(system, "audit", reject)
    response = client.post(
        "/api/system/reset", json={"password": "definitiva1", "confirmation": "APAGAR"}
    )
    ...
    monkeypatch.setattr(system, "audit", original)
    assert (
        client.post(
            "/api/system/reset",
            json={"password": "definitiva1", "confirmation": "APAGAR"},
...
def test_synthetic_storage_data_is_bounded_and_removed(client, monkeypatch):
    ...
    with TestClient(app) as teacher:
        teacher.headers["X-Ponto"] = "1"
        teacher.post(
            "/api/login", json={"login": "storage_teacher", "password": "definitiva1"}
        )
    ...
    with transaction() as session:
        assert (
            session.scalar(select(Setting).where(Setting.key.like("storage-test:%")))
            is None
        )
